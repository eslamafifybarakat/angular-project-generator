...
import { ArchitectureStepComponent } from './architecture-step.component';
import { ProjectConfigService } from '../../application/project-config.service';

function root(fixture: ComponentFixture<ArchitectureStepComponent>): HTMLElement {
  return fixture.nativeElement as HTMLElement;
}

function patternButtons(fixture: ComponentFixture<ArchitectureStepComponent>): HTMLButtonElement[] {
  return Array.from(root(fixture).querySelectorAll<HTMLButtonElement>('.options > .option'));
}

function treeLines(fixture: ComponentFixture<ArchitectureStepComponent>): string[] {
  return Array.from(root(fixture).querySelectorAll('.tree__row .mono')).map(
    (el) => el.textContent?.trim() ?? '',
  );
}

describe('ArchitectureStepComponent', () => {
  let fixture: ComponentFixture<ArchitectureStepComponent>;
  let configuration: ProjectConfigService;

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [ArchitectureStepComponent],
      providers: [provideZonelessChangeDetection()],
    }).compileComponents();
    fixture = TestBed.createComponent(ArchitectureStepComponent);
